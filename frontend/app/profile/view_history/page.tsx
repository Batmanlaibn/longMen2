...
export default function ViewHistory({
  ...
  // ✅ CHAPTER ONLY
  const chapterHistory = sortedHistory.filter(
    (h) => h.type === "chapter"
  );
    ...
    <>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold">
          Таны үзсэн хичээлүүд ({chapterHistory.length})
        </h3>
          ...
          >
            <div className="flex justify-between gap-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    removeHistoryItem(item.id, item.courseId || 0)
                  }
                ...
                <button
                  onClick={() =>
                    router.push(
                      `/cards/${item.courseId}?chapter=${item.id}`
                    )
                  }
